...
export default function JoinCoordination() {
  ...
  return (
    ...
    >
            ...
            {derived_status === 'expired_unconfirmed' && (
              <div style={{ backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', padding: '20px', borderRadius: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <span style={{ color: '#64748b', fontSize: 15, lineHeight: 1.5 }}>
                  {t('coordination.expired_msg', 'Las fechas propuestas ya pasaron. No es posible enviar disponibilidad.')}
                </span>
              </div>
            )}
      ...
      {derived_status === 'expired_unconfirmed' && (
        <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, padding: '20px 20px', background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderTop: '1px solid rgba(15,23,42,0.05)', boxShadow: '0 -4px 24px rgba(0,0,0,0.04)', paddingBottom: 'calc(20px + env(safe-area-inset-bottom))', zIndex: 10, textAlign: 'center' }}>
          <span style={{ color: '#475569', fontWeight: 600, fontSize: 15 }}>{t('coordination.expired_title', 'Coordinación finalizada')}</span>
        </div>
      )}
    </ScreenContainer>
  );
