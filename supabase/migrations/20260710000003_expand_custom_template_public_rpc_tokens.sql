...
BEGIN
    ...
    -- Validar que el tema sea custom y tenga el prefijo correcto
    IF v_encuentro.tema_invitacion != 'custom' OR v_encuentro.invitation_template NOT LIKE 'custom_%' THEN
        RETURN NULL;
    END IF;

    -- Extraer el UUID real del diseño
    BEGIN
        v_template_id := replace(v_encuentro.invitation_template, 'custom_', '')::uuid;
    EXCEPTION WHEN invalid_text_representation THEN
        RETURN NULL;
    END;

    -- Buscar el diseño activo
