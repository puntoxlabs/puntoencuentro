-- ============================================================
-- Migración: add_date_coordination_base_rpcs
-- ============================================================

BEGIN;

-- ============================================================
-- 1. Crear Encuentro con Opciones
-- ============================================================
...
BEGIN
    ...
    -- Inicializar arrays para parseo server-side sin tablas temporales
    ...
    FOR v_op IN SELECT * FROM jsonb_array_elements(p_opciones) LOOP
        ...
        -- Construir timestamptz asumiendo que las fechas dadas por el frontend local son locales a Argentina.
        -- Nota: Esto se documenta como MVP y puede requerir ajuste para internacionalización futura.
        v_op_ts := (v_op_fecha + v_op_hora) AT TIME ZONE 'America/Argentina/Buenos_Aires';

        -- Validación retrospectiva estricta
        IF v_op_ts <= now() THEN
            RETURN json_build_object('ok', false, 'error', 'option_in_past');
        END IF;

        -- Detección de duplicados en el array (fecha y hora exacta)
    ...
    -- Obtener la alternativa temporal más temprana para validación del deadline
    ...
    -- 6. Generación server-side estricta de identificadores (ignorando cliente)
    ...
    INSERT INTO public.encuentros (
        ...
        date_mode, coordination_status, response_deadline, fecha, hora, selected_option_id
    )
    VALUES (
        v_enc_id,
        v_titulo,
        ...
        v_link_virtual,
        v_tipo_invitacion,
        v_user_id, -- El UUID extraído inviolablemente de JWT
        v_token,
        'activo',
        v_tema,
        v_tema_invitacion,
...
-- ============================================================
-- 2. Lectura Host (Coordinación)
-- ============================================================
CREATE OR REPLACE FUNCTION public.get_coordinacion_host_seguro(p_encuentro_id uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
-- ============================================================
-- 3. Lectura Participante Individual (Coordinación)
-- ============================================================
CREATE OR REPLACE FUNCTION public.get_coordinacion_participante_seguro(p_token text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
    ...
    RETURN json_build_object(
        ...
        'encuentro', json_build_object(
            ...
            'tema', v_enc.tema,
            'estado', v_enc.estado,
            -- link_virtual omitido explícitamente en la fase de coordinación para el participante
            'tema_invitacion', COALESCE(v_enc.tema_invitacion, 'classic'),
            'invitation_template', v_enc.invitation_template
        ),
...
-- ============================================================
-- 4. Lectura Pública (Coordinación)
-- ============================================================
CREATE OR REPLACE FUNCTION public.get_coordinacion_publica_seguro(p_public_token text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
